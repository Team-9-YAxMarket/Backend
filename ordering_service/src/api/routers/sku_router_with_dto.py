...
async def create_order(order: OrderRequest):
    ...
    order_info: Order = Order(order.orderId,order.items)
    #sku_items: dict[str, SKU]

    check_skus_url = "http://127.0.0.1:8080/check_skus/status/"
    for item in order.items:
        order_info.items[item.sku] = SKU(**item)

    # sku_items = [{"sku": item.sku, "count": item.count} for item in
    #              order.items]
    check_skus_request = {"items": asdict(order_info.items)}
    ...
    get_sku_info_request = {"items": order.items}
    ...
    for item in sku_info:
        sku = item['sku']
        order_info.items[sku].length = item["length"]
        order_info.items[sku].width = item["width"]
        order_info.items[sku].height = item["height"]
        order_info.items[sku].weight = item["weight"]
        order_info.items[sku].barcode = item["barcode"]
        order_info.items[sku].cargotypes = item["cargotypes"]
